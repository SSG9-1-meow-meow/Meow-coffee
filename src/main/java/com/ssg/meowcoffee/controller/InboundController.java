...
public class InboundController {
  ...
  @GetMapping("/api")
  @ResponseBody
  public ResponseEntity<Map<String, Object>> getInboundListData(@AuthenticationPrincipal CustomUserDetails customUserDetails, InboundCriteria criteria) {
    log.info("GET /inbounds/api - 입고 목록 데이터 API 요청. Criteria: {}", criteria);

    // Spring security 권한 설정
    String currentUserId = customUserDetails.getUserId();
    UserRole currentUserRole = customUserDetails.getUserRole();
  ...
  // --- [회원] 입고 요청 처리 API
  @PostMapping("/req")
  public String createInboundRequest(@AuthenticationPrincipal CustomUserDetails customUserDetails,
                                     @Valid @RequestBody InboundReqInputDTO inputDTO,
                                       RedirectAttributes redirectAttributes) {

    log.info("POST /inbounds/req - 신규 입고 요청 처리 시작: {}", inputDTO);

    // 서버에서 요청 시각 및 아이디를 설정
    inputDTO.set_inDttmReq(LocalDateTime.now());
    inputDTO.set_comId(customUserDetails.getUserId());
  ...
  @GetMapping("/{inReqId}")
  public String inboundRequestDetailPage(@AuthenticationPrincipal CustomUserDetails customUserDetails,
                                         @PathVariable long inReqId, Model model) {
    log.info("GET /inbounds/{} - 입고 요청 상세 페이지 렌더링 요청", inReqId);

    // --- TODO: 실제 로그인한 사용자 정보로 대체 => Complete! ---
    UserRole currentUserRole = customUserDetails.getUserRole();
  ...
  @PutMapping("/{inReqId}")
  @ResponseBody
  public ResponseEntity<Map<String, String>> modifyInboundRequest(@AuthenticationPrincipal CustomUserDetails customUserDetails,
                                                                  @PathVariable long inReqId,
                                                                  @Valid @RequestBody InboundReqInputDTO inputDTO) {
  ...
  @ResponseBody
  public ResponseEntity<Map<String, String>> cancelInboundRequest(@AuthenticationPrincipal CustomUserDetails customUserDetails,
          @PathVariable long inReqId) {
    log.info("DELETE /inbounds/{} - 입고 요청 취소 처리 시작", inReqId);

    // TODO: 실제 로그인한 사용자 정보 사용 => complete!
    String currentUserId = customUserDetails.getUserId();
    UserRole currentUserRole = customUserDetails.getUserRole();
  ...
  public ResponseEntity<Map<String, String>> finalizeInboundItem(
    ...
    // TODO: processDTO.setManagerId() -> 실제 로그인한 관리자 ID로 설정 => Complete!
    processDTO.setManagerId(customUserDetails.getUserId());
