...
| 기능 분류 | 주요 기능 | 상세 설명                                                                                                                                                          |
| --- | --- |----------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 로그인 관련 | 로그인 | - 입력한 아이디, 비밀번호에 해당하는 회원의 계정으로 창고관리시스템에 접속<br/>- 로그인한 회원의 정보는 로그아웃할 때까지 로그인 세션에 저장                                                                             |
|  | 로그아웃 | 현재 로그인한 회원의 정보를 로그인 세션에서 삭제하고 로그인 페이지로 이동                                                                                                                      |
|  | 회원등록 신청 | - 선택한 회원가입유형에 따라 회원정보 입력 폼을 작성한 후 제출<br/>- 회원등록 신청 폼 제출 시 해당 회원의 상태는 ‘승인대기’                                                                                    |
|  | 아이디 찾기 | 입력한 사업자등록번호(거래처, 배송기사)/직원명(관리자)과 이메일이 모두 일치하는 계정의 아이디를 조회                                                                                                      |
...
| 회원관리(관리자용) | 회원리스트 조회/회원 검색 | - 승인/미승인 회원들의 목록을 조회<br/>- 관리자는 각 회원별 아이디, 회원유형, 이름, 회원상태, 회원등록일, 마지막 로그인 일자를 조회<br/>- 회원리스트 상단의 검색필터(회원권한, 계정상태, 키워드검색, 기간)을 적용하여 WMS에 등록된 승인/미승인 회원의 현황 파악 가능 |
  |  | 회원정보 상세조회 | - 회원리스트에서 특정 회원에 관한 관리자 전용 상세정보 조회 기능 제공                                                                                                                   |
  | 회원관리(총관리자 전용) | 계정상태 변경 | - 회원리스트에서 선택한 회원의 계정상태 변경 가능<br/>- 승인대기 상태인 계정을 승인완료 상태로 전환하거나, 휴면대기 상태인 회원을 휴면회원 상태로 변경할 때 사용
  |  | 휴면회원 전환 | 회원리스트에서 지정한 회원의 계정상태를 휴면회원 상태로 변경합니다. 이는 회원리스트에서 조회된 회원이 마지막 로그인 날짜로부터 1년 이상 WMS에 접속하지 않았을 경우에 수행할 수 있습니다.                                                     |
