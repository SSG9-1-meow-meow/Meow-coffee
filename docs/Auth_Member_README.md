...
- 회원가입 기능을 통해, 회원가입 유형에 따라 Meow Coffee WMS가 제공하는 주요 서비스를 이용할 계정을 생성할 수 있습니다.
- 가입이 승인된 회원은 아이디와 비밀번호를 입력하여 로그인한 후, 자신의 회원유형에 따라 Meow Coffee WMS가 제공하는 서비스를 이용할 수 있습니다.
...
| 구분              | 기술                                     | 상세 설명                                  |
|:----------------|:---------------------------------------|:---------------------------------------|
| **Backend**     | Java 17, Spring Framework 5.3.27       | Spring Framework 기반 서버/비즈니스 로직 실행 및 관리 |
| **Security**    | Spring Security 5.8.16                 | 필터 기반 인증/인가 서비스를 제공하는 Spring 모듈        |
| **Database**    | MySQL Connector 8.0.22, HikariCP 5.0.0 | MySQL 연결 및 DB 커넥션 풀 관리                 |
| **Persistence** | MyBatis 3.5.7, MyBatis Spring 2.0.6    | SQL 매퍼를 사용한 DB 데이터 영속성 관리              |
| **Server**      | Apache Tomcat 9.0.11                   | 웹 애플리케이션 서버(WAS)                       |
| **Frontend**    | JSP, JavaScript, Axios, jstl           | 서버사이드 렌더링 & 동적 UI 사용                   |
| **UI/UX**       | Bootstrap 5                            | 반응형 웹 디자인 제공                           |
